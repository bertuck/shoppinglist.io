...
const CACHE_URLS = ['public/index.html', 'public/index.jsx'];
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.url.startsWith(self.location.origin)) {
    event.respondWith(async function () {
      ...
      if (cachedResponse) {//console.log(cachedResponse);
      }

      if (cachedResponse) return cachedResponse; // If we dont have it in catch, let's fetch and cache it.

      const cache = await caches.open(RUNTIME);
      const response = await fetch(event.request); // Put a copy of the response in the runtime cache.
